...
{
    Import-LocalizedData -BindingVariable localizedData -filename VMSwitch.psd1 `
...
<#
.SYNOPSIS
    Gets cVMSwitch resource current state.
.PARAMETER Name
    Specifies a name of the VM switch. This is a mandatory parameter.
.PARAMETER Type
    Specifies the Type of the VM switch. This has three possible values: External, Internal, and Private. This is a mandatory parameter.
#>
...
<#
.SYNOPSIS
    Sets cVMSwitch resource to desired state.
.PARAMETER Name
    Specifies a name of the VM switch. This is a mandatory parameter.
.PARAMETER Type
    Specifies the Type of the VM switch. This has three possible values: External, Internal, and Private. This is a mandatory parameter.
.PARAMETER NetAdaterName
    Specifies the name(s) of the physial network adapters that will be used in the creation of an external VM switch.
    Specifying more than one network adapter will result in creation of a Switch Embedded Team (SET).
.PARAMETER AllowManagementOS
    Specifies if a network adapter should be added to the host OS and connect it to the VM switch.
.PARAMETER EnableIoV
    Specifies if IoV should be enbled. This cannot be used along with EnablePacketDirect or when the MinimumBandwidthMode is not None.
.PARAMETER MinimumBandwidthMode
    Specifies the minimum bandwidth mode for the VM switch. Allowed values are 'None', 'Default', 'Weight', 'Absolute'.
    This cannot be changed once the switch is provisioned.
.PARAMETER TeamingMode
    Specifies the TeamingMode configuration for SET based VM Switch.
    Only allowed value is SwitchIndependent.
.PARAMETER LoadBalancingAlgorithm
    Specifies the load balancing algorithm for the VM Switch. Allowed Values are 'Dynamic','HyperVPort'.
.PARAMETER EnablePacketDirect
    Specifies if Packet Direct should be enabled for the VM switch. This cannot be used along with EnableIoV.
.PARAMETER Ensure
    Specifies if the VM switch configuration should be present or removed. The possible argument values are Present and Absent.
#>
...
<#
.SYNOPSIS
    Tests if the cVMSwitch resource is in desired state.
.PARAMETER Name
    Specifies a name of the VM switch. This is a mandatory parameter.
.PARAMETER Type
    Specifies the Type of the VM switch. This has three possible values: External, Internal, and Private. This is a mandatory parameter.
